...
bool Image_IMWrite(const char* filename, Mat img) { return cv::imwrite(filename, *img); }

bool Image_IMWrite_WithParams(const char* filename, Mat img, IntVector params) {
    std::vector<int> compression_params;
    for (int i = 0, *v = params.val; i < params.length; ++v, ++i) { compression_params.push_back(*v); }
...
Mat Image_IMDecode(ByteArray buf, int flags) {
    ...
    for (size_t i = 0; i < buf.length; i++) { data.push_back(buf.data[i]); }
